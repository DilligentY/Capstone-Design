...
import open3d.visualization as o3v 
...
def point_cloud_video(executor, rgbd_frame, mdata, timestamp, o3dvis : o3d.visualization.O3DVisualizer):
    ...
    #                 rgbd_frame.depth)
    app = o3v.gui.Application.instance
...
def pcd_video_from_camera(executor : ThreadPoolExecutor, o3dvis : o3d.visualization.O3DVisualizer):
    ...
    while True:
        # Frame Captrue
        rgbd_frame = rscam.capture_frame(wait=True)
        rgbd_frame.color = rgbd_frame.color.clone()
        rgbd_frame.depth = rgbd_frame.depth.clone()
        # Convert From RGBD Images to Point Clouds
        point_cloud_video( 
            executor,
            rgbd_frame,
            mdata,
...
def main():
    ...
    # RealSense Camera Registration
    have_cam = o3d.t.io.RealSenseSensor.list_devices()
    have_bag = (len(sys.argv) == 2)

    with ThreadPoolExecutor(max_workers=4) as executor:
        # Run IO and compute in side threadpool
        if have_bag:
